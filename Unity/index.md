...
Scriptable Render Pipeline パッケージのどれかを使用している場合は、ポストプロセスパッケージは含まれていすので、既にインストールされています。
...
シェーダーグラフを使用してシェーダーを作る方法です。

Assets メニューを開き、 Create > Shader > PBR Graph を選択して新しいシェーダーを作成します。

単一ノードの新しいシェーダーグラフが表示されます。
Save Asset をクリックすると、シェーダーをマテリアルで使用できるようになります。

***

### シェーダーグラフを使用したグローイングエフェクトの作成

シェーダーグラフを使用してグローイングエフェクトを作成してみます。

* 新しい SRP シェーダーグラフアセットを作成します。
* 何もないスペースを右クリックするか、スペースバーを押して　Create Node メニューを開きます。
* Fresnel Effect ノードを見つけます。

Fresnel Effect ノードは白い光沢を作成します。色を付けることも可能です。

* 新しい Color ノードを作成します。
* Multiply ノードを作成します(Math > Basic > Multuply)
* Color ノードの Out スロットをクリックし、 Multiply ノードの入力の 1 つにドラッグします。
* Fresnel Effect の Out スロットをクリックして、 Multiply ノードの他の入力にドラッグします。
* 最後に、 Multiply ノードの Out スロットを PBR Master ノードの Emission スロットに接続します。

***

### シェーダーグラグからのプロパティー公開

シェーダーグラフを完全に新規に作成する代わりに、プロパティーを公開して変更可能にします。

プロパティーを定義するには、まず Blackboard ペインを使用してプロパティーを作成する必要があります。
Blackboard には、シェーダーに関連付けられているすべてのプロパティー一覧が表示されます。

* シェーダーの Blackboard ペインの右上にある + ボタンをクリックします。
* 作成するプロパティーのタイプを選択すると、新しいプロパティーが作成されます。シェーダーグラフにドラッグするとノードとして表示されます。

プロパティーノードがシェーダーグラフに追加されると、それを他のノードの入力に接続して他の値と同じように使用できます。
違いは、プロパティーがマテリアルから制御可能になることです。

Blackboard ペインでプロパティーをダブルクリックすると、プロパティー名を編集できます。
シェーダーグラフの既存の入力ノードを右クリックしてプロパティーノードに変換することも可能です。この機能を使って、作成時は固定値にし、後にそれらをプロパティーとして公開できるので、シェーダーの作成が楽になります。
